...
app = FastAPI(title="T.A.R. API", version=VERSION)
app.add_middleware(
    CORSMiddleware,
...
class NodeRequest(BaseModel):
    name: str = Field(min_length=1, max_length=500)
    endpoint: str = Field(default="", max_length=3_000)
...
def ready():
    try:
        ...
        return {"ready": True, "database": True, "version": VERSION}
...
async def ask(req: AskRequest, identity: dict = Depends(require_identity)):
    try:
        result = await answer(req.query, research=req.research, workspace_id=identity["workspace_id"])
        audit(
            "ask.completed",
            "query",
            "",
            {"research": req.research, "source_count": len(result.get("sources", []))},
...
async def tasks(req: TaskRequest, identity: dict = Depends(require_identity)):
    try:
        result = await execute_task(req.kind, req.prompt, req.options, workspace_id=identity["workspace_id"])
...
async def search(req: SearchRequest, identity: dict = Depends(require_identity)):
    ...
    stored = remember(results, workspace_id=identity["workspace_id"]) if req.persist else 0
    audit(
        "search.completed",
        "search",
        "",
        {"sources": req.sources, "results": len(results), "stored": stored},
        workspace_id=identity["workspace_id"],
        actor=identity["label"],
    )
...
def memory_recall(req: RecallRequest, identity: dict = Depends(require_identity)):
    ...
    audit(
        ...
        "memory",
        "",
        {"results": len(results), "source": req.source},
        workspace_id=identity["workspace_id"],
        actor=identity["label"],
    )
...
def memory_stats(identity: dict = Depends(require_identity)):
    return stats(identity["workspace_id"])
...
async def ingest_file(
    ...
    try:
        ...
        audit(
            ...
            {"filename": file.filename, "source": source, "chunks": result.get("chunks", 0)},
            workspace_id=identity["workspace_id"],
            actor=identity["label"],
        )
        return result
...
async def csv_tool(file: UploadFile = File(...), identity: dict = Depends(require_identity)):
    try:
        result = analyze_csv(await file.read(), file.filename or "data.csv", identity["workspace_id"])
        audit(
            "tool.csv",
            "artifact",
...
async def image_tool(file: UploadFile = File(...), identity: dict = Depends(require_identity)):
    try:
        result = image_metadata(await file.read(), file.filename or "image", identity["workspace_id"])
        audit(
            "tool.image_metadata",
            "artifact",
            result["artifact"]["artifact_id"],
...
def documents(limit: int = 100, identity: dict = Depends(require_identity)):
    return {"documents": list_documents(max(1, min(limit, 500)), identity["workspace_id"])}


@app.get("/v1/artifacts")
def artifacts(limit: int = 100, identity: dict = Depends(require_identity)):
    return {"artifacts": list_artifacts(max(1, min(limit, 500)), identity["workspace_id"])}
