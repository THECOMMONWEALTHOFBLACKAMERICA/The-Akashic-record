...
def parse_bytes(filename: str, data: bytes) -> tuple[str, list[tuple[int | None, str]], dict]:
    ...
    if suffix == ".epub":
        epub_meta, chapters = parse_epub(data)
        return "application/epub+zip", chapters, {"epub": epub_meta}
...
def ingest_bytes(filename: str, data: bytes, *, title: str | None = None, source: str = "upload", source_uri: str = "", metadata: dict | None = None, workspace_id: str = "default") -> dict:
    ...
    with Session(engine) as session:
        ...
        session.add(IngestJob(id=job_id, workspace_id=workspace_id, status="processing", title=title or filename, source=source))
        session.commit()
    try:
        media_type, page_texts, parsed_metadata = parse_bytes(filename, data)
        effective_metadata = {**(metadata or {}), **parsed_metadata}
        effective_title = title or (parsed_metadata.get("epub", {}).get("title") if isinstance(parsed_metadata.get("epub"), dict) else None) or filename
        document_id = uuid.uuid4().hex
        chunk_rows = []
        ordinal = 0
        for page, text in page_texts:
            for start, end, chunk in _chunk_text(text):
                chunk_rows.append(ChunkRecord(document_id=document_id, workspace_id=workspace_id, ordinal=ordinal, text=chunk, start_char=start, end_char=end, page=page, metadata_json=json.dumps({"filename": filename, **effective_metadata}, ensure_ascii=False)))
                ordinal += 1
        with Session(engine) as session:
            session.add(DocumentRecord(document_id=document_id, workspace_id=workspace_id, title=effective_title, source=source, source_uri=source_uri, media_type=media_type, sha256=digest, metadata_json=json.dumps(effective_metadata, ensure_ascii=False)))
            session.add_all(chunk_rows)
            job = session.get(IngestJob, job_id)
            job.status = "completed"
            job.title = effective_title
            job.document_id = document_id
            job.chunks = len(chunk_rows)
            job.finished_at = datetime.now(timezone.utc)
            session.commit()
        return {"job_id": job_id, "status": "completed", "document_id": document_id, "chunks": len(chunk_rows), "sha256": digest, "media_type": media_type, "metadata": effective_metadata}
...
def list_documents(limit: int = 100, workspace_id: str = "default") -> list[dict]:
    ...
    return [{"document_id": d.document_id, "title": d.title, "source": d.source, "source_uri": d.source_uri, "media_type": d.media_type, "sha256": d.sha256, "metadata": json.loads(d.metadata_json or "{}"), "created_at": d.created_at.isoformat() if d.created_at else None} for d in docs]
