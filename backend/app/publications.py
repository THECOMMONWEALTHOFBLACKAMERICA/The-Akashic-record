...
def publish_artifact(artifact_id: str, workspace_id: str) -> dict:
    ...
    manifest = manifest_for_artifact(
        ...
        artifact_cid=artifact_ipfs["cid"],
        version=VERSION,
    )
    ...
    with Session(engine) as session:
        session.add(
            PublicationRecord(
                ...
                artifact_cid=artifact_ipfs["cid"],
                manifest_cid=manifest_ipfs["cid"],
                sha256=row.sha256,
            )
        )
        session.commit()
    ...
    return {
        ...
        "artifact_id": artifact_id,
        "artifact_cid": artifact_ipfs["cid"],
        "artifact_gateway_url": artifact_ipfs["gateway_url"],
        "manifest_cid": manifest_ipfs["cid"],
        "manifest_gateway_url": manifest_ipfs["gateway_url"],
        "sha256": row.sha256,
        "deduplicated": False,
    }
...
def latest_publication(artifact_id: str, workspace_id: str) -> dict | None:
    with Session(engine) as session:
        row = session.scalar(
            ...
            .where(
                PublicationRecord.artifact_id == artifact_id,
                PublicationRecord.workspace_id == workspace_id,
            )
...
def list_publications(workspace_id: str, limit: int = 100) -> list[dict]:
    with Session(engine) as session:
        rows = session.scalars(
            select(PublicationRecord)
            .where(PublicationRecord.workspace_id == workspace_id)
            .order_by(PublicationRecord.id.desc())
            .limit(limit)
        ).all()
...
def _serialize(row: PublicationRecord) -> dict:
    ...
    return {
        ...
        "workspace_id": row.workspace_id,
        "artifact_id": row.artifact_id,
        "artifact_cid": row.artifact_cid,
        "artifact_gateway_url": f"{gateway_url()}/{row.artifact_cid}",
        "manifest_cid": row.manifest_cid,
        "manifest_gateway_url": f"{gateway_url()}/{row.manifest_cid}",
        "sha256": row.sha256,
