...
class CaseUpdate(BaseModel):
    status: str | None = None; restricted_research: bool | None = None; legal_hold: bool | None = None; retention_policy: str | None = Field(default=None, max_length=200)
class CaseResearch(BaseModel):
    query: str = Field(min_length=1, max_length=2000); include_dawes: bool = False; broaden_web: bool = False; persist_as_evidence: bool = True; limit: int = Field(default=5, ge=1, le=25)
class EvidenceCreate(BaseModel):
    title: str = Field(min_length=1, max_length=1000); source_tier: int = Field(ge=1, le=3); source: str = Field(default="", max_length=300); source_uri: str = Field(default="", max_length=3000); citation: str = ""; retrieval_metadata: dict = Field(default_factory=dict); original_filename: str = Field(default="", max_length=1000); original_sha256: str = Field(default="", max_length=64); uploader: str = Field(default="", max_length=500); claimed_provenance: str = ""; media_type: str = Field(default="application/octet-stream", max_length=200); transformations: list = Field(default_factory=list); derived_artifacts: list = Field(default_factory=list)
class EvidenceReview(BaseModel):
    status: str; review_notes: str = ""; exclusion_reason: str = ""
class CaseDelete(BaseModel):
    policy_basis: str = Field(min_length=1, max_length=2000)
class AccessGrant(BaseModel):
    key_id: str = Field(min_length=1, max_length=64); role: str = Field(default="readonly", max_length=40)


def _require(case_id: str, identity: dict, *, write: bool = False, review: bool = False) -> None:
    if not has_case_access(case_id, identity["workspace_id"], identity["key_id"], write=write, review=review):
...
def patch_case(case_id: str, req: CaseUpdate, identity: dict = Depends(require_authenticated_identity)):
    ...
    try: return update_case(case_id, identity["workspace_id"], status=req.status, restricted_research=req.restricted_research, legal_hold=req.legal_hold, retention_policy=req.retention_policy, actor=identity["label"])
    except (KeyError, ValueError) as exc: raise HTTPException(status_code=400 if isinstance(exc, ValueError) else 404, detail=str(exc)) from exc
...
async def case_research(case_id: str, req: CaseResearch, identity: dict = Depends(require_authenticated_identity)):
    _require(case_id, identity, write=True)
    try: return await research_case(case_id, identity["workspace_id"], req.query, include_dawes=req.include_dawes, broaden_web=req.broaden_web, persist_as_evidence=req.persist_as_evidence, actor=identity["label"], limit=req.limit)
...
def case_export(case_id: str, identity: dict = Depends(require_authenticated_identity)):
    ...
    try: return export_case(case_id, identity["workspace_id"])
    except KeyError as exc: raise HTTPException(status_code=404, detail=str(exc)) from exc

@router.delete("/cases/{case_id}")
def remove_case(case_id: str, req: CaseDelete, identity: dict = Depends(require_authenticated_identity)):
    _require(case_id, identity, write=True)
    try: return delete_case(case_id, identity["workspace_id"], actor=identity["label"], policy_basis=req.policy_basis)
    except KeyError as exc: raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def grant_access(case_id: str, req: AccessGrant, identity: dict = Depends(require_authenticated_identity)):
    if not has_case_access(case_id, identity["workspace_id"], identity["key_id"], write=True): raise HTTPException(status_code=403, detail="Commission case access denied")
    try:
...
async def upload_evidence(case_id: str, file: UploadFile = File(...), title: str = Form(default=""), source_tier: int = Form(default=1), source: str = Form(default="applicant_submission"), claimed_provenance: str = Form(default=""), citation: str = Form(default=""), identity: dict = Depends(require_authenticated_identity)):
    ...
    if len(data) > 100 * 1024 * 1024: raise HTTPException(status_code=413, detail="File exceeds 100 MB Commission upload limit")
    try:
...
def evidence(case_id: str, identity: dict = Depends(require_authenticated_identity)):
    _require(case_id, identity)
    try: return {"evidence": list_evidence(case_id, identity["workspace_id"])}
    except KeyError as exc: raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def review(evidence_id: str, req: EvidenceReview, identity: dict = Depends(require_authenticated_identity)):
    # Resolve evidence through accessible case lists rather than trusting an ID alone.
    allowed = accessible_case_ids(identity["workspace_id"], identity["key_id"])
    matching_case = None
    for case_id in allowed:
        if any(e["evidence_id"] == evidence_id for e in list_evidence(case_id, identity["workspace_id"])):
            matching_case = case_id; break
    if not matching_case or not has_case_access(matching_case, identity["workspace_id"], identity["key_id"], review=True): raise HTTPException(status_code=403, detail="Commission reviewer access required")
    try: return review_evidence(evidence_id, identity["workspace_id"], status=req.status, reviewer=identity["label"], review_notes=req.review_notes, exclusion_reason=req.exclusion_reason)
    except KeyError as exc: raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
