...
async def generate_image(prompt: str, *, size: str = "1024x1024") -> dict:
    ...
    data = await _image_from_response(body)
    return save_artifact("generated-image.png", data, "image/png", {"provider_model": model, "prompt": prompt})


async def edit_image(image: bytes, prompt: str, *, filename: str = "image.png") -> dict:
    endpoint = os.getenv("TAR_IMAGE_EDIT_API_URL", "").rstrip("/")
    key = os.getenv("TAR_IMAGE_API_KEY", "")
    model = os.getenv("TAR_IMAGE_EDIT_MODEL", "") or os.getenv("TAR_IMAGE_MODEL", "")
    if not endpoint or not model:
        raise ProviderNotConfigured("Configure TAR_IMAGE_EDIT_API_URL and an image edit model.")
    async with httpx.AsyncClient(timeout=300) as client:
        r = await client.post(endpoint, headers=_auth(key), data={"model": model, "prompt": prompt}, files={"image": (filename, image, "application/octet-stream")})
        r.raise_for_status(); body = r.json()
    data = await _image_from_response(body)
    return save_artifact("edited-image.png", data, "image/png", {"provider_model": model, "prompt": prompt, "source_filename": filename})


async def transcribe_audio(audio: bytes, *, filename: str = "audio.wav", language: str | None = None) -> dict:
    endpoint = os.getenv("TAR_TRANSCRIBE_API_URL", "").rstrip("/")
    key = os.getenv("TAR_TRANSCRIBE_API_KEY", "")
    model = os.getenv("TAR_TRANSCRIBE_MODEL", "")
    if not endpoint or not model:
        raise ProviderNotConfigured("Configure TAR_TRANSCRIBE_API_URL and TAR_TRANSCRIBE_MODEL.")
    fields = {"model": model}
    if language:
        fields["language"] = language
    async with httpx.AsyncClient(timeout=600) as client:
        r = await client.post(endpoint, headers=_auth(key), data=fields, files={"file": (filename, audio, "application/octet-stream")})
        r.raise_for_status(); body = r.json()
    text = body.get("text") or body.get("transcript")
    if not text:
        raise RuntimeError("Transcription provider returned no transcript")
    artifact = save_artifact(filename + ".transcript.txt", text.encode("utf-8"), "text/plain", {"provider_model": model, "source_filename": filename})
...
async def generate_video(prompt: str, *, seconds: int = 8) -> dict:
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        ...
        start.raise_for_status(); job = start.json()
        result_url = job.get("url") or job.get("output_url")
        status_url = job.get("status_url")
        if not result_url and status_url:
            for _ in range(120):
                await asyncio.sleep(2)
                status = await client.get(status_url, headers=headers); status.raise_for_status(); payload = status.json()
